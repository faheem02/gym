-- =====================================================================
-- Gym Management System — COMPLETE DATABASE SCHEMA (fresh install / live)
-- Base schema + all migrations merged. Compatible with MySQL 5.7+, MySQL 8.0+, MariaDB 10.2+
--
-- HOW TO IMPORT (cPanel / Live Server):
--   1) Create a new database in cPanel -> MySQL Databases
--      (e.g. atrmarke_Gym_portal) and attach a user with ALL PRIVILEGES.
--   2) Open phpMyAdmin, SELECT that database (from left sidebar).
--   3) Go to "Import" tab -> choose this database_full.sql file -> click "Go" / "Import".
--
...
-- ---------------------------------------------------------------------
-- Drop existing tables (safe for clean re-import)
-- ---------------------------------------------------------------------
...
CREATE TABLE members (
    ...
    status ENUM('active', 'inactive') DEFAULT 'active',
    access_type ENUM('gym','kids_play','both') NOT NULL DEFAULT 'gym',
    registration_fee DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    monthly_fee DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    trainer_fee DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    kids_fee DECIMAL(10,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE member_options (
    id INT AUTO_INCREMENT PRIMARY KEY,
    category ENUM('membership_type','area_of_interest','fitness_goal') NOT NULL,
...
INSERT INTO trainers (name, phone, email, specialty, fee) VALUES
('John Doe', '9876543200', 'john@example.com', 'Strength Training', 3000.00),
('Maria Khan', '9876543201', 'maria@example.com', 'Yoga & Pilates', 2500.00);
...
INSERT INTO members (name, phone, email, join_date, status, access_type, registration_fee, monthly_fee, trainer_fee, kids_fee, discount, trainer_id) VALUES
('Rahul Sharma', '9876543210', 'rahul@example.com', '2026-01-10', 'active', 'gym', 1000.00, 3000.00, 0.00, 0.00, 0.00, 1),
('Priya Patel', '9876543211', 'priya@example.com', '2026-02-15', 'active', 'both', 1000.00, 3000.00, 2500.00, 1500.00, 500.00, 2),
('Amit Kumar', '9876543212', 'amit@example.com', '2026-03-20', 'active', 'gym', 1000.00, 3000.00, 0.00, 0.00, 0.00, 1),
('Sneha Reddy', '9876543213', 'sneha@example.com', '2026-04-05', 'inactive', 'kids_play', 1000.00, 0.00, 0.00, 2000.00, 0.00, NULL);
...
(1, '2026-09-01', '06:30:00'),
(2, '2026-09-01', '07:00:00'),
(3, '2026-09-01', '08:15:00');
