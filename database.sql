...
ALTER TABLE members ADD FOREIGN KEY (trainer_id) REFERENCES trainers(id) ON DELETE SET NULL;
...
CREATE TABLE canteen_purchase_items (
    ...
    FOREIGN KEY (product_id) REFERENCES canteen_products(id)
) ENGINE=InnoDB;
...
CREATE TABLE canteen_sale_items (
    ...
    FOREIGN KEY (sale_id) REFERENCES canteen_sales(id) ON DELETE CASCADE,
    FOREIGN KEY (product_id) REFERENCES canteen_products(id)
) ENGINE=InnoDB;
...
CREATE TABLE canteen_supplier_payments (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (supplier_id) REFERENCES canteen_suppliers(id)
) ENGINE=InnoDB;
