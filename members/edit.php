...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '' || $phone === '' || $join_date === '') {
        ...
        try {
            ...
            $stmt = $pdo->prepare('UPDATE members SET name = ?, phone = ?, email = ?, date_of_birth = ?, gender = ?, membership_type = ?, area_of_interest = ?, join_date = ?, status = ?, trainer_id = ? WHERE id = ?');
            $stmt->execute([$name, $phone, $email ?: null, $date_of_birth, $gender, $membership_type, $area_of_interest, $join_date, $status, $trainer_id > 0 ? $trainer_id : null, $id]);
...
<div class="card form-card">
    <div class="card-body">
        ...
        <form method="POST" action="">
                    ...
                    <?php foreach ($areasOfInterest as $aoi): ?>
                        <div class="col-md-6">
                            <div class="form-check">
                                <?php $checked = in_array($aoi, $currentAOI) ? 'checked' : ''; ?>
                                <input class="form-check-input" type="checkbox" name="area_of_interest[]" value="<?php echo htmlspecialchars($aoi); ?>" id="aoi_<?php echo md5($aoi); ?>" <?php echo $checked; ?>>
            ...
            <div class="section-label mb-3 mt-4">
                <h6 class="fw-bold text-muted"><i class="fas fa-clipboard-list me-1"></i> Assign Plan
                    <?php if ($currentSub): ?>
